    ...
    , fl_state(false), fr_state(false), bl_state(false), br_state(false), led_state(false)
{
    ...
    // 串口收到温度数据的信号，到更新 UI Label 的槽函数
    connect(serialManager, &SerialPortManager::temperatureReceived,
            this, &MainWindow::handleTemperatureUpdate);

    // 串口收到哨兵数据的信号，到更新 UI Label 的槽函数
    connect(serialManager, &SerialPortManager::sentrywarningReceived,
            this, &MainWindow::handleSentryWarningUpdate);

    // MQTT 收到云端的合法命令信号，直接把它怼给串口的发送槽函数！
    connect(mqttManager, &MqttManager::cloudCommandReceived,
            serialManager, &SerialPortManager::sendCommand);

    // 串口收到 STM32 的温度信号
    connect(serialManager, &SerialPortManager::temperatureReceived,
            mqttManager, &MqttManager::publishTemperature);

    // 3. 初始化串口 (连接开发板上的 ttymxc5)
...
void MainWindow::on_bt_fl_clicked()
{
    ...
    if (fl_state) {
        serialManager->sendCommand("@Servo_fl!#"); // 开门指令
    }
    else {
        serialManager->sendCommand("@Servo_fc!#"); // 关门指令
    }
}
...
void MainWindow::on_bt_fr_clicked()
{
    fr_state = !fr_state;
    if (fr_state) {
        serialManager->sendCommand("@Servo_fr!#");
    }
    else {
        serialManager->sendCommand("@Servo_fc!#"); // 前门关闭指令
    }
}
...
void MainWindow::on_bt_bl_clicked()
{
    bl_state = !bl_state;
    if (bl_state) {
        serialManager->sendCommand("@Servo_bl!#");
    }
    else {
        serialManager->sendCommand("@Servo_bc!#"); // 后门关闭指令
    }
}
...
void MainWindow::on_bt_br_clicked()
{
    br_state = !br_state;
    if (br_state) {
        serialManager->sendCommand("@Servo_br!#");
    }
    else {
        serialManager->sendCommand("@Servo_bc!#");
    }
}

void MainWindow::on_bt_led_clicked()
{
     serialManager->sendCommand("@LED!#");
}
