...
void Serial_Init(void)
{
	...
	GPIO_Init(SERIAL_GPIO_PORT, &GPIO_InitStructure);					//将PA9引脚初始化为复用推挽输出
	
	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_IPU;
	GPIO_InitStructure.GPIO_Pin = SERIAL_RX;
	GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;
	GPIO_Init(SERIAL_GPIO_PORT, &GPIO_InitStructure);					//将PA10引脚初始化为上拉输入
	...
	USART_InitStructure.USART_Parity = SERIAL_PARITY;		//奇偶校验，不需要
	USART_InitStructure.USART_StopBits = SERIAL_STOPBITS;	//停止位，选择1位
	...
	NVIC_InitStructure.NVIC_IRQChannelPreemptionPriority = 11;		//指定NVIC线路的抢占优先级为7，大于configMAX_SYSCALL_INTERRUPT_PRIORITY
